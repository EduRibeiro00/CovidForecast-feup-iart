...
def determine_moves_neutron_soldier(current_board, minimax_board, size):
    ...
    for i in range(size):
        for j in range(size):
            ...
            if minimax_board[i][j] != current_board[i][j] and minimax_board[i][j] != NEUTRON_CHAR and current_board[i][j] != NEUTRON_CHAR:
                if current_board[i][j] == BLANK_SPACE_CHAR:
                    found_new_soldier = True
                    new_soldier = (i, j)
                elif minimax_board[i][j] == BLANK_SPACE_CHAR:
